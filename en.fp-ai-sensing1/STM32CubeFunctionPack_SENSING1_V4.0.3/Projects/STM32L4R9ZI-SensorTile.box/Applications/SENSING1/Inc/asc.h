...
#define AI_ASC_IN_NUM       (1)
#define AI_ASC_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 30, 32, 1, 1, NULL), \
}
#define AI_ASC_IN_SIZE { \
  (30 * 32 * 1), \
}
#define AI_ASC_IN_1_SIZE  (30 * 32 * 1)
#define AI_ASC_IN_1_SIZE_BYTES  ((30 * 32 * 1) * 1)


#define AI_ASC_OUT_NUM      (1)
#define AI_ASC_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8, 1, 1, 3, 1, NULL), \
}
#define AI_ASC_OUT_SIZE { \
  (1 * 1 * 3), \
}
#define AI_ASC_OUT_1_SIZE  (1 * 1 * 3)
#define AI_ASC_OUT_1_SIZE_BYTES  ((1 * 1 * 3) * 1)
 ...
 * @ref ai_asc_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_ASC_*
 * defines into file @ref asc.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
